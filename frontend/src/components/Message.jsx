...
const Message = ({ content, sender, time, isLoading = false, onDownload, hasDownloadButton = false, filesToDownload = [] }) => {
  ...
  const bubbleClasses = isUser
    ? (theme === 'dark'
        ? 'bg-hiremind-accent-purple text-hiremind-text-dark-primary'
        : 'bg-hiremind-accent-purple/20 text-hiremind-text-light-primary')
    : (theme === 'dark'
        ? 'bg-hiremind-element-dark text-hiremind-text-dark-primary'
        : 'bg-hiremind-element-light text-hiremind-text-light-primary');

  const timeClasses = theme === 'dark' ? 'text-hiremind-text-dark-secondary' : 'text-hiremind-text-light-secondary';

 return (
    <div className={`flex mb-5 ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div
        className={`max-w-[70%] md:max-w-[60%] px-3 py-4 md:px-4 md:py-7 rounded-none shadow-sm backdrop-blur-sm transition-colors duration-200 
