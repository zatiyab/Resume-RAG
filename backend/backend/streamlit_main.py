...
def get_relevant_docs(user_query, collection, filter_data=True, k=5):
    ...
    for result in results['points']:
        ...
        if filter_data: 
            ...
            data = result['payload'].get('history', '') # For history collection, use 'history' payload

        if data:
            all_data += str(data)
            all_data += '\n\n'
        else:
            print(f"--- WARNING: No 'page_content' or 'history' found for ID {res_id} or it's empty. ---")
...
def main(k, user_query, is_jd_search=False):
    ...
    retrieved_resume_context = get_relevant_docs(user_query=user_query, collection='resumes', filter_data=True, k=k)
    ...
    if ("he" in user_query) or ("his" in user_query) or ("him" in user_query) or ("her" in user_query) or ("she" in user_query) or ("they" in user_query) or ("their" in user_query):
        ...
        # final_history_for_llm_prompt = actual_last_history_items[0].payload['history']
        print('No user')
        # print('History given to LLM:',final_history_for_llm_prompt)
...
If fewer candidates match than requested, provide only the exact number found. If none are found, state this clearly.
    ...
    if "retrieval_result" in st.session_state and st.session_state.retrieval_result["files"]:
        ...
        folder_path = "/home/anubhav/svn_workspace/Resume_RAG/resumes" # Verify this path is correct
